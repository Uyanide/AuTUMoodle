        ...
        try:
            with ZipFile(self._file_path, 'r') as zip_ref:
                for zip_info in zip_ref.infolist():
                    ...
                    # Try to find matching config with full filename
                    file_config = self._find_matching_config(category_name, entry_name, file_download_configs)
                    
                    # If not found and not a subdirectory, try without extension
                    if file_config is None and len(splitted) == 2:
                    ...
                    destination_path = file_config.directory / original_entry_name
                    ...
                    local_date = destination_path.stat().st_mtime if destination_path.exists() else 0
